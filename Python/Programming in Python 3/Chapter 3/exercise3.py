...
def print_users(users):
    ...
    for key in sorted(users):
        ...
        if flag % 2 != 0:
            leftline = "{0:.<{nw}} ({1.id:4}) {1.username:{uw}}".format(name, user, nw=namewidth, uw=usernamewidth)
        else:
            print (leftline + " " + "{0:.<{nw}} ({1.id:4}) {1.username:{uw}}".format(name, user, nw=namewidth, uw=usernamewidth))
